<template>
  <div id="add">
    ...
    <h1 style="margin-bottom: 5%">Editar {{this.nombres}} {{this.apellidos}}</h1>
    <div class="field">
...
export default {
  ...
  methods:{
    ...
    async getEmpleadoId(){
      ...
      try {
        const response = await axios.get(`http://192.168.0.8:3000/empleado?id=${id}`);
        console.log(response.data);
        this.dpi_old = response.data.empleado[0].dpi;
        this.dpi = response.data.empleado[0].dpi;
        this.nombres = response.data.empleado[0].nombres;
        this.apellidos = response.data.empleado[0].apellidos;
        this.fecha_nacimiento = new Date(response.data.empleado[0].fecha_nacimiento).toLocaleDateString('en-CA');
        this.sueldo = response.data.empleado[0].sueldo;
        this.ubicacion = response.data.empleado[0].ubicacion_id;
    ...
    async guardarEmpleado(){
      try {
        await axios.put(`http://192.168.0.8:3000/empleado?id=${this.dpi_old}`,
            {
              dpi: this.dpi,
